  ...
  forbidden: [
    ...
    },
    {
      ...
      from: {
        path: "^(src)",
        pathNot:
          "(^src/.*/(?:test|tests|__tests__)/)|(?:[.](?:spec|test|cases|bench)|[Tt]estUtils|[Tt]estHarness)[.](?:js|mjs|cjs|jsx|ts|mts|cts|tsx)$",
      },
      to: {
        ...
        dependencyTypesNot: ["type-only"],
        pathNot: ["node_modules/@types/", "node_modules/vite/types/"],
      },
    },
    {
    ...
    {
      name: "engine-stays-generic",
      severity: "error",
      comment:
        "Engine is the generic core: it must not import game-specific, UI, or app-shell code. " +
        "Keeps the engine reusable and headless-runnable (required by the deterministic balancing runner).",
      from: { path: "^src/engine" },
      to: { path: "^src/(game|ui|app-shell)" },
    },
    {
      name: "data-is-leaf",
      severity: "error",
      comment:
        "Data holds schemas/blueprints: it must not import engine/game/ui/app-shell. " +
        "Schemas are a leaf the other layers depend on, not the reverse.",
      from: { path: "^src/data" },
      to: { path: "^src/(engine|game|ui|app-shell)" },
    },
    {
      name: "utils-is-pure-leaf",
      severity: "error",
      comment:
        "utils/ are pure leaf helpers (including the determinism primitives pseudoRandom/worldSeed): " +
        "they must not import any other src layer.",
      from: { path: "^src/utils" },
...
// generated: dependency-cruiser@17.4.0 on 2026-05-18T08:14:46.380Z
