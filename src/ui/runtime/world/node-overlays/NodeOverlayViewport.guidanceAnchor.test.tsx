...
describe("NodeOverlayViewport guidance anchor", () => {
    beforeEach(() => {
    ...
    it("anchors bottom node guidance below its target", async () => {
        const runtime = {
        ...
        const callout = screen.getByTestId("runtime-guidance-callout");
        const transform = normalize(
            globalThis.getComputedStyle(callout).transform,
        );
        expect(callout.dataset.guidanceAnchor).toBe("below");
        expect(transform).toContain(
            "translate3d(120px,100px,0)translate(-50%,0)",
        );
    });
});
