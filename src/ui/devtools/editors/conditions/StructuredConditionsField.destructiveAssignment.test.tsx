...
describe("StructuredConditionsField destructive assignment", () => {
    beforeEach(() => {
    ...
    it("switches a row to destructive assignment and stores the new kind", () => {
        render(
            <ThemeProvider>
                <PortalManager>
        ...
        fireEvent.change(screen.getByDisplayValue("fact_threshold"), {
            target: { value: "destructive_assignment_has_all_bodies" },
        });
        const draft = useSessionStore.getState().sessions[filename]
            ?.draft as any;

        expect(
            screen.getByText(/holds every extant non-aggregate body/i),
        ).toBeDefined();
        expect(
            draft.blueprints[blueprintId]._editor.abilities
                .conditionalActivation.conditions[0].kind,
        ).toBe("destructive_assignment_has_all_bodies");
    });
});
