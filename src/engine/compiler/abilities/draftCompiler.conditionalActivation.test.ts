            ...
            _editor: {
                abilities: {
                    ...
                    conditionalActivation: targetIds.map((targetId) => ({
                        ...
                        targets: [{ ability: "draft", targetId }],
                    })),
                },
            },
        }),
...
describe("draftCompiler conditionalActivation", () => {
    it("appends one active-state ref per matching conditional activation", () => {
        const activeRefs = (targetIds: string[]) =>
            compile(targetIds)
                .components.behavior?.rules?.find(
                    (entry) => entry.id === "sys_draft_pool_0",
                )
        ...
        expect(activeRefs(["draft-1"])).toEqual(refs(1));
        expect(activeRefs(["draft-1", "draft-1"])).toEqual(refs(2));
